...
class _ForgotPasswordEmailState extends State<ForgotPasswordEmail> {
  ...
  // 1. دالة زر "Send Reset Code" -> تنقلك لصفحة الـ Verification (التي أرفقت صورتها)
  void _handleSendCode() async {
    if (_formKey.currentState!.validate()) {
      setState(() => _isLoading = true);
      
      // محاكاة إرسال الإيميل (تأخير ثانية واحدة)
      await Future.delayed(const Duration(seconds: 1));
      
      if (!mounted) return;
      setState(() => _isLoading = false);

      // الانتقال لصفحة إدخال الكود (ForgotPasswordVerify)
      Navigator.push(
        context,
  ...
  // 2. دالة زر "Find by mobile number" -> تنقلك لصفحة رقم الهاتف
  void _handleFindByNumber() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const ForgotPasswordNumber()),
    );
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              // الزر الأول: إرسال الكود للإيميل
              SizedBox(
              ...
              // الزر الثاني: الانتقال لصفحة رقم الهاتف
              SizedBox(
                width: double.infinity,
                height: 54,
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.white, // جعلنا اللون أبيض لتمييزه
                    foregroundColor: const Color(0xFFCC3333),
                    side: const BorderSide(color: Color(0xFFCC3333)), // إطار أحمر
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                    elevation: 0,
                  ),
                  child: const Row(
